...
### Key Subsystems
- **Layers System**: The scene is divided into a `Layer[]` (background, mid-back, mid-fore, foreground). Each layer manages its own entities and parallax scrolling effects based on `cameraX`.

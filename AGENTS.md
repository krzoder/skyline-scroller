...
Use the installed `~/.agents/skills/session-harness/SKILL.md` for substantive work.
The strongest available model of the actual hosting provider plans and manages;
resolve current models and effort at runtime. Workers normally use medium effort,
gathering uses low, and ordinary independent reviews use medium. Only the manager
starts bounded workers or the two other provider-family reviews. Leaf tasks never
restart orchestration. Do not duplicate conversation history or launch maximum-effort
execution sessions. Trivial work does not need another review cycle.

Honor the owner's configured quota, money and context policy across sessions and
computers. Never bypass a stop by switching accounts or providers. Use one verified
shared authority when accounts span machines; local ledgers do not coordinate them.
No mandatory reserve is imposed by Markdown. Preserve setup, grants and daily usage.
Use `ai-session configure` for guided settings, `ai-session version` to inspect the
runtime, and `ai-session update` for an explicit release update. Development `main`
does not automatically replace installed releases. Keep private setup out of commits.
Project-specific release, data and acceptance boundaries below remain in force.
